...
class stackoverflow_engine:
    ...
    def query_page(self):
        ...
        print(api_url)
        response = self.session.get(api_url).json()
        nextpage = 2
        yield response
        while nextpage <= 10 and response['has_more']:
            response = self.session.get(api_url, params={'page': nextpage}).json()
            nextpage += 1
            yield response
...
class github_engine:    
    ...
    def query_page(self):
        ...
        if response.status_code != 200:
            print('Rate limit reached, please try in one minute!')
            return None
...
class search_engine:
    ...
    def search(self):
        ...
        merged = {'created_date' : date.today(), 'items' : {}}
